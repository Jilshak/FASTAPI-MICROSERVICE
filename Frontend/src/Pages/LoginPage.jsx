...
function LoginPage() {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const request = await axios.post('http://127.0.0.1:8000/auth/token/', credentials, {
                headers: {
            ...
            if (request.status === 200) {
                let token = jwtDecode(response.access_token)
                if (token.sub != null && token.id != null) {
                    localStorage.setItem('fastauth', response.access_token)
                    await navigate('/')
                }
            }
        } catch (error) {
            alert("Server Under Maintanace please be patient!!!")
                    ...
                    <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                            ...
                            <form class="space-y-4 md:space-y-6" action="#">
                                ...
                                <button onClick={(e) => handleLogin(e)} type="submit" class="w-full text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Login</button>
                                <p class="text-sm text-center font-light text-gray-500 dark:text-gray-400">
                                    Don’t have an account yet? <Link to='/register'><span class="font-medium cursor-pointer text-blue-600 hover:underline dark:text-blue-500">Sign up</span></Link>
                                </p>
                            </form>
                        </div>
                    </div>
